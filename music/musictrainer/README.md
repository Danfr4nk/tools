...
3. **Score** — for each track: a 1.00–10.00 number, plus one verdict — `HATE` (hate it) / `LIKE` (alright, not playlist-worthy, saved for stats) / `PLAYLIST` (love it, want to hear it again). Click the track to load its Spotify embed player, or listen in your own Spotify. Keyboard: `j`/`k` move, `1`/`2`/`3` score, `p` loads the player.
